...
import numpy as np
import sys
from models.models_snli import embedding_linear
from models.models_snli import encoder
from models.models_snli import atten
...
def train(args):
    ...
    if args.share_params > 0:
        input_encoder.input_linear2.weight.data.copy_(input_encoder.input_linear1.weight.data)
        for key in [1, 4]:
            inter_atten.mlp_f2._modules[str(key)].weight.data.copy_(
                inter_atten.mlp_f1._modules[str(key)].weight.data)
            inter_atten.mlp_f2._modules[str(key)].bias.data.copy_(
                inter_atten.mlp_f1._modules[str(key)].bias.data)
            inter_atten.mlp_g2._modules[str(key)].weight.data.copy_(
                inter_atten.mlp_g1._modules[str(key)].weight.data)
            inter_atten.mlp_g2._modules[str(key)].bias.data.copy_(
                inter_atten.mlp_g1._modules[str(key)].bias.data)
    ...
    for k in range(args.epoch):
        ...
        for i in range(len(train_batches)):
            ...
            # if train_lbl_batch.data.size(0) == 1:
            #     # simple sample batch
            #     train_src_batch = torch.unsqueeze(train_src_batch, 0)
            #     train_tgt_batch = torch.unsqueeze(train_tgt_batch, 0)
            ...
            if args.share_params > 0:
                input_encoder.input_linear1.weight.grad.data.add_(
                    input_encoder.input_linear2.weight.grad.data)
                input_encoder.input_linear2.weight.grad.data.zero_()
                for key in [1, 4]:
                    inter_atten.mlp_f1._modules[str(key)].weight.grad.data.add_(
                        inter_atten.mlp_f2._modules[str(key)].weight.grad.data)
                    inter_atten.mlp_f1._modules[str(key)].bias.grad.data.add_(
                        inter_atten.mlp_f2._modules[str(key)].bias.grad.data)
                    inter_atten.mlp_f2._modules[str(key)].weight.grad.data.zero_()
                    inter_atten.mlp_f2._modules[str(key)].bias.grad.data.zero_()

                    inter_atten.mlp_g1._modules[str(key)].weight.grad.data.add_(
                        inter_atten.mlp_g2._modules[str(key)].weight.grad.data)
                    inter_atten.mlp_g1._modules[str(key)].bias.grad.data.add_(
                        inter_atten.mlp_g2._modules[str(key)].bias.grad.data)
                    inter_atten.mlp_g2._modules[str(key)].weight.grad.data.zero_()
                    inter_atten.mlp_g2._modules[str(key)].bias.grad.data.zero_()

            grad_norm = 0.
            para_norm = 0.

            grad_norm += input_encoder.input_linear1.weight.grad.data.norm() ** 2
            para_norm += input_encoder.input_linear1.weight.data.norm() ** 2

            for key in [1, 4]:
                grad_norm += inter_atten.mlp_f1._modules[str(key)].weight.grad.data.norm() ** 2
                grad_norm += inter_atten.mlp_f1._modules[str(key)].bias.grad.data.norm() ** 2
                para_norm += inter_atten.mlp_f1._modules[str(key)].weight.data.norm() ** 2
                para_norm += inter_atten.mlp_f1._modules[str(key)].bias.data.norm() ** 2

                grad_norm += inter_atten.mlp_g1._modules[str(key)].weight.grad.data.norm() ** 2
                grad_norm += inter_atten.mlp_g1._modules[str(key)].bias.grad.data.norm() ** 2
                para_norm += inter_atten.mlp_g1._modules[str(key)].weight.data.norm() ** 2
                para_norm += inter_atten.mlp_g1._modules[str(key)].bias.data.norm() ** 2

                grad_norm += inter_atten.mlp_h._modules[str(key)].weight.grad.data.norm() ** 2
                grad_norm += inter_atten.mlp_h._modules[str(key)].bias.grad.data.norm() ** 2
                para_norm += inter_atten.mlp_h._modules[str(key)].weight.data.norm() ** 2
                para_norm += inter_atten.mlp_h._modules[str(key)].bias.data.norm() ** 2

            grad_norm += inter_atten.final_linear.weight.grad.data.norm() ** 2
            grad_norm += inter_atten.final_linear.bias.grad.data.norm() ** 2
            para_norm += inter_atten.final_linear.weight.data.norm() ** 2
            para_norm += inter_atten.final_linear.bias.data.norm() ** 2
            ...
            if args.share_params > 0:
                input_encoder.input_linear2.weight.data.copy_(
                    input_encoder.input_linear1.weight.data)
                for key in [1, 4]:
                    inter_atten.mlp_f2._modules[str(key)].weight.data.copy_(
                        inter_atten.mlp_f1._modules[str(key)].weight.data)
                    inter_atten.mlp_f2._modules[str(key)].bias.data.copy_(
...
if __name__ == '__main__':
    ...
    parser.add_argument('--share_params', help='share parameters',
                        type=int, default=1)
